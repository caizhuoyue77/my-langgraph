...
## 目录
1. [API记录表](docs/API_docs.md)
2. [Bug记录表](docs/bugs.md)
3. [To Do List](docs/todos.md)

## 706待办
1. ~~【重要】跑通开发者模式和傻瓜模式~~
2. ~~【重要】跑通从cache面板=》调用API的流程~~
3. ~~功能：在API显示页面显示tools.json的内容~~
   1. 添加一列是API状态，对于能用的标绿，不能用的标红。
   2. 添加API测试按钮，点击之后就发送请求，测试API是否能用。
4. 笨工作：给每个API都添加默认参数，这样无论如何都不会报错。
5. 重构：修改API的存储方式（API.json）的存储方式
   1. 笨功能：把每个API的参数都写到tools.json里面
6. 重构：修改API的管理方式（调用方式），不要一个API一个python文件了
   1. 需要有一个统一的入口函数（所有调用API的代码都经过）
      1. 存在问题：后处理函数不一样 解决方案：先不修改已有的API，但是对于新添加的API，暂时不进行后处理，或者是简单截断。
7. 功能：实现动态添加、测试API，然后把API的信息存储到tools.json里面
