...
class GoogleNewsEntertainmentInput(BaseModel):
    language: str = Field(default="en-US", description="用于获取Google News娱乐新闻的语言代码。")
...
async def google_news_entertainment_iter(language: str, max_items: int) -> dict:
    """
    Asynchronously retrieves entertainment news from Google News using specified parameters via the RapidAPI service.
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return {"error": f"Failed to retrieve entertainment news, status code: {response.status_code}"}
    except requests.RequestException as e:
        return {"error": f"Request failed: {str(e)}"}

def google_news_entertainment(language: str = "en-US", max_items: int = 5) -> dict:
    """
    A synchronous wrapper function to retrieve entertainment news from Google News.
    ...
    """
    return asyncio.run(google_news_entertainment_iter(language, max_items))

if __name__ == "__main__":
    news_language = "en-US"  # Replace with your desired language code
    max_items = 5  # Replace with your desired number of items
    google_news_results = google_news_entertainment(news_language, max_items)
    print("Google News Entertainment Results:", google_news_results)
